...
def generate_url():
    formatted_query = "pantai sanur"
    formatted_query = formatted_query.replace(" ", "+")  # Ganti spasi dengan +
    return f"https://www.detik.com/search/searchall?query={formatted_query}"
...
def crawl_detik():
    ...
    for link, title in articles.items():
        ...
        print(f"Processed Article  - {title}: {link}")
        time.sleep(2)
